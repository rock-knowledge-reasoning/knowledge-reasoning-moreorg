...
public:
    ...
    OPCall(const std::string& op_name,
    ...
    static double sum(const Agent& agent,
                      const owlapi::model::IRI& dataproperty,
                      const OrganizationModelAsk& ask);
    static double mean(const Agent& agent,
                       const owlapi::model::IRI& dataproperty,
                       const OrganizationModelAsk& ask);

    double evalComposite(const Agent& agent,
                         const OrganizationModelAsk& ask) const;
    double evalAtomic(const Agent& agent,
                      const OrganizationModelAsk& ask) const;
...
public:
    ...
    void loadByName(const owlapi::model::IRI& ruleName,
                    const OrganizationModelAsk& ask);
     ...
     */
    static InferenceRule::Ptr
    loadPropertyCompositeAgentRule(const owlapi::model::IRI& dataproperty,
                                   const OrganizationModelAsk& ask);

    static InferenceRule::Ptr
    loadCompositeAgentRule(const owlapi::model::IRI& ruleName,
                           const OrganizationModelAsk& ask);
     ...
     *
     * A plain rule is always applied, independently of dealing with atomic or
     * composite agents
     */
    static InferenceRule::Ptr
    loadPropertyAtomicAgentRule(const owlapi::model::IRI& dataproperty,
                                const OrganizationModelAsk& ask);

    static InferenceRule::Ptr
    loadAtomicAgentRule(const owlapi::model::IRI& ruleName,
                        const OrganizationModelAsk& ask);

    /**
     * Preparing the rule, means to instantiate the function by substituting
     * placeholder with correspondingly bound function calls/calls to inbuilt
     * operations \see OPCall
     */
    void prepare();
    ...
    void addBinding(const owlapi::model::IRI& placeholder,
                    const owlapi::model::IRI& name)
    {
        mBindings[placeholder] = name;
    }
...
protected:
    ...
    static std::map<owlapi::model::IRI, InferenceRule::Ptr>
        mPropertyCompositeAgentRules;
    static std::map<owlapi::model::IRI, InferenceRule::Ptr>
        mPropertyAtomicAgentRules;

    static std::map<owlapi::model::IRI, InferenceRule::Ptr>
        mCompositeAgentRules;
