...
Resource::Resource(const owlapi::model::IRI& model,
    ...
    : mModel(model)
    , mPropertyConstraints(propertyConstraints.begin(),
                           propertyConstraints.end())
{
}
...
bool Resource::operator==(const Resource& other) const
{
    return mModel == other.mModel &&
           mPropertyConstraints == other.mPropertyConstraints;
}
...
void Resource::addPropertyConstraints(
...
void Resource::merge(const Resource& other) { *this = merge(*this, other); }
...
Resource Resource::merge(const Resource& a, const Resource& b)
{
    if(a.mModel != b.mModel)
    {
        throw std::invalid_argument(
            "moreorg::Resource::merge: cannot merge resources: "
            " models are different: '" +
            a.mModel.toString() + "' and '" + b.mModel.toString());
    }
...
Resource::Set Resource::merge(const Resource::Set& resourcesA,
                              const Resource::Set& resourcesB)
{
