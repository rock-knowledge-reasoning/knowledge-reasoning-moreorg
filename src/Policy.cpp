...
Policy::computeSharesByType(const owlapi::model::IRI& property,
                            ...
                            const OrganizationModelAsk& ask) const
{
    ...
    if(modelPool.empty())
    {
        throw std::invalid_argument(
            "owlapi::model::Policy::computeSharesByType"
                                ...
                                const OrganizationModelAsk& ask)
{
                                   ...
                                   vocabulary::OM::resolve("SelectionPolicy")))
    {
        policies::SelectionPolicy::Ptr selectionPolicy =
            make_shared<policies::SelectionPolicy>(policyName);
        for(size_t i = 0; i < MAX_POLICY_ELEMENTS; ++i)
        {
            ...
            IRIList instances =
                ask.ontology().allRelatedInstances(policyName,
                                                   indexPlaceholder);
            if(instances.size() == 0)
            {
                break;
                              ...
                              "FunctionalityBasedSelection")))
            {
                ...
                policies::SelectionPolicy::Ptr fbs =
                    make_shared<policies::FunctionalityBasedSelection>(
                        functionality);
                selectionPolicy->add(fbs);
            } else if(ask.ontology().isInstanceOf(
                          policyElement,
                          vocabulary::OM::resolve("PropertyBasedSelection")))
            {
                OWLAnnotationValue::Ptr selectByValue =
                    ask.ontology().getAnnotationValue(
                        policyElement,
                ...
                IRIList operators = ask.ontology().allRelatedInstances(
                    policyElement,
                    vocabulary::OM::resolve("hasOperator"));

                IRI operatorName = operators[0];

                policies::SelectionPolicy::Ptr pbs =
                    make_shared<policies::PropertyBasedSelection>(property,
                                                                  operatorName);
            ...
            {
                IRISet relations =
                    ask.ontology().getRelatedDataProperties(policyElement);
                if(relations.size() != 1)
                {
                    throw std::runtime_error(
                        "moreorg::Policies::getInstance '" +
                ...
                OWLLiteral::Ptr literal =
                    ask.ontology().getDataValue(policyElement, relation, false);
