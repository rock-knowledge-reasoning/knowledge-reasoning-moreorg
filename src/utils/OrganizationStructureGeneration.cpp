#include "OrganizationStructureGeneration.hpp"
...
{
    Coalition::const_iterator cit = list.begin();
    os << "[";
...
std::string Bounds::toString() const
{
    std::stringstream ss;
    ss.precision(10);
    ss << "Bounds: max[" << maximum << "] min[" << minimum << "] average["
       << average << "]";
    return ss.str();
}
...
CoalitionStructureGeneration::Statistics::remainingIntegerPartitions() const
{
    ...
    std::vector<numeric::IntegerPartition> blacklist(pruned.size() +
                                                     searched.size());
    it = std::set_union(pruned.begin(),
                        pruned.end(),
                        searched.begin(),
                        searched.end(),
                        blacklist.begin());
    ...
    std::vector<numeric::IntegerPartition> remaining(all.size() +
                                                     blacklist.size());
    it = std::set_difference(all.begin(),
                             all.end(),
                             blacklist.begin(),
                             blacklist.end(),
                             remaining.begin());
...
std::string CoalitionStructureGeneration::Statistics::toString() const
{
    ...
    ss << "        all: " << IntegerPartitioning::toString(allIntegerPartitions)
       << std::endl;
    ss << "        pruned: "
       << IntegerPartitioning::toString(prunedIntegerPartitions) << std::endl;
    ss << "        searched: "
       << IntegerPartitioning::toString(searchedIntegerPartitions) << std::endl;
    std::vector<numeric::IntegerPartition> remain =
        remainingIntegerPartitions();
    ss << "        remaining: " << IntegerPartitioning::toString(remain)
       << std::endl;
    return ss.str();
}

CoalitionStructureGeneration::CoalitionStructureGeneration(
    const AtomicAgent::List& agents,
...
{
    using namespace numeric;
    ModelPool modelPool = AtomicAgent::getModelPool(mAgents);
    LimitedCombination<owlapi::model::IRI> combinations(modelPool,
                                                        mAgents.size(),
                                                        MAX);
    do
    {
        ...
        AtomicAgent::List agents = AtomicAgent::toList(m);

        mAgentCoalitionMap[coalition.size()].insert(agents);
    ...
    IntegerPartitioning::PartitionsMap::const_iterator cit =
        partitionsMap.begin();
    ...
    IntegerPartitionBoundsMap updatedIntegerPartitionsBoundMap =
        prune(mIntegerPartitionBoundsMap);
}

double
CoalitionStructureGeneration::bestLowerBound(const CoalitionBoundMap& boundMap)
{
    ...
    for(; cit != boundMap.end(); ++cit)
    {
        ...
        if(bounds.minimum > lowerBound)
        {
            lowerBound = bounds.minimum;
        }
    }
    return lowerBound;
}

double CoalitionStructureGeneration::bestLowerBound(
    const CoalitionStructureGeneration::IntegerPartitionBoundsMap& boundMap)
{
    double lowerBound = std::numeric_limits<double>::min();
    ...
    // lower bound is minimum the current best solution value
...
double CoalitionStructureGeneration::bestUpperBound(
    const IntegerPartitionBoundsMap& boundMap)
{
    ...
    // Upper bound is always at least the current best solution value
    double best = currentBestSolutionValue();
    if(upperBound < best)
    {
        return best;
...
Bounds CoalitionStructureGeneration::computeIntegerPartitionBounds(
    const IntegerPartition& partition)
{
    Bounds bounds;
    bounds.maximum = std::numeric_limits<double>::min();
    bounds.average = 0;
    bounds.minimum = std::numeric_limits<double>::max();
    ...
    for(; cit != partition.end(); ++cit)
    {
        ...
        bounds.maximum = std::max(bounds.maximum, coalitionBound.maximum);
        bounds.average += coalitionBound.average / partitionSize;
        bounds.minimum = std::min(bounds.minimum, coalitionBound.minimum);
    }

    return bounds;
}

CoalitionStructureGeneration::IntegerPartitionBoundsMap
CoalitionStructureGeneration::prune(const IntegerPartitionBoundsMap& boundsMap)
{
    ...
    for(; cit != boundsMap.end(); ++cit)
    {
        ...
        {
            LOG_DEBUG_S << "Removing partition: "
                        << IntegerPartitioning::toString(cit->first)
                        << " -- max: " << bounds.maximum
                        << " < global min: " << globalLowerBound;
...
CoalitionStructureGeneration::CoalitionBoundMap
CoalitionStructureGeneration::prune(const CoalitionBoundMap& boundMap)
{
    ...
    for(; cit != boundMap.end(); ++cit)
    {
        ...
        if(bounds.maximum < globalLowerBound)
        {
            LOG_DEBUG_S << "Removing partition: " << cit->first
                        << " -- max: " << bounds.maximum
                        << " < global min: " << globalLowerBound;
...
CoalitionStructure CoalitionStructureGeneration::findBest(double quality)
{
    ...
    //// prune space where a partitions upper bound is smaller than global lower
    ///bound
    ...
    while(true)
    {
        ...
        LOG_INFO_S << "Compute best coalition structure for this subspace: "
                   << IntegerPartitioning::toString(partition);
        {
            boost::unique_lock<boost::mutex> lock(mStatisticsMutex);
            mStatistics.searchedIntegerPartitions.push_back(partition);
        }
        bool improvedResult = searchSubspace(partition,
                                             0,
                                             0,
                                             mAgents,
                                             CoalitionStructure(),
        ...
        } else
        {
            ...
            {
                LOG_INFO_S << "Good enough coalition found: "
                           << currentBestSolution() << ", value "
                           << currentBestSolutionValue();
                break;
            }
            ...
            LOG_DEBUG_S << "Best coalition found so far: "
                        << currentBestSolution() << ", " << bounds;

            // TODO: pick the next one for evaluation and repeat until satified
            // with the solution
    ...
    LOG_DEBUG_S << "Best coalition found: " << currentBestSolution() << ", "
                << currentBestSolutionValue();
...
void CoalitionStructureGeneration::anytimeSearch(double quality)
{
    ...
    mThread =
        boost::thread(&CoalitionStructureGeneration::findBest, this, quality);
}

void CoalitionStructureGeneration::stopSearch() { mThread.interrupt(); }
...
IntegerPartition CoalitionStructureGeneration::selectIntegerPartition(
    const CoalitionStructureGeneration::IntegerPartitionBoundsMap& boundMap,
    double maximumBound) const
{
    ...
    {
        LOG_INFO_S << "Current: " << IntegerPartitioning::toString(partition)
                   << ", current value " << value;
        LOG_INFO_S << "Check: " << IntegerPartitioning::toString(it->first)
                   << ", max value " << it->second.maximum << ", min value "
                   << it->second.minimum << ", avg value "
                   << it->second.average;
        // Use average as tiebreaker
        if(it->second.maximum > value ||
           (it->second.maximum == value && it->second.average > average))
        {
            partition = it->first;
    ...
    if(value == 0.0)
    {
        throw std::runtime_error(
            "CoalitionStructureGeneration::selectIntegerPartition: No integer "
            "partition found greater than the given maximum bound");
    } else
    {
        return partition;
    }
    ...
    // consider betaStar > 1, where betaStar = 1.05 "means that the solution
    // sought needs to hava a value that is at least 95% of the optimal one"
    //
    // select to search the smallest sub-space that could, potentially, give a
    // value greater than or equal to (globalUpperBound/betaStar)
    //
    //      Select G (the IntegerPartition) = argumin_{G \in \mathcal{G'}:
    //      UpperBound_G \gt \frac{globalUpperBound}{{betaStar} (|P_{g}|)
    //
    // |P_{G}| -> is the size of the number of coalition structures / integer
    // partitions for the given number of coalitions
}

/**
 *
 * \param globalUpperBound
 * \param bestStar Quality of the solution, i.e. 1.05 means 95% percent of the
 * optimal solution
 */
bool CoalitionStructureGeneration::searchSubspace(
    const IntegerPartition& partition,
    size_t k,
    size_t alpha,
    const AtomicAgent::List& agents,
    ...
    double betaStar)
{
    ...
    LOG_DEBUG_S << indent << " search subspace of current structure: "
                << currentStructure;

    if(k > 0 && partition[k] != partition[k - 1])
    {
        LOG_DEBUG_S << indent << " search subspace for k: " << k
    ...
    for(size_t i = 0; i < k; ++i)
    {
        upperBoundM_k +=
            partition[i] *
            IntegerPartitioning::multiplicity(partition, partition[i]);
    }
    LOG_DEBUG_S << indent << " upperBound M_k: " << upperBoundM_k;

    // Compute upper bound of subspace, so that we can stop computation when
    // this maximum has been reached
    double upperBoundOfSubspace = mIntegerPartitionBoundsMap[partition].maximum;
    LOG_DEBUG_S << indent << " upperBound of subspace "
                << IntegerPartitioning::toString(partition) << ": "
                << upperBoundOfSubspace;
    ...
    do
    {
        ...
        LOG_DEBUG_S << indent << " current combination m_k="
                    << IntegerPartitioning::toString(m_k)
                    << ", alpha=" << alpha;
        // m_k[0] + 1: we start with index 0, but the algorithmic description
        // uses 1 as first index
        if(((int)alpha) <= m_k[0] + 1 && m_k[0] + 1 <= upperBoundM_k)
        {
            LOG_DEBUG_S << indent << " computing combination";
            // Map combination of indices to current coalition, i.e. an agent
            // (sub)set
            AtomicAgent::List remainingAgents = agents;
            AtomicAgent::List coalition;
            for(size_t i = 0; i < m_k.size(); ++i)
            {
                AtomicAgent agent = agents[m_k[i]];
                coalition.push_back(agent);
                std::vector<AtomicAgent>::iterator it =
                    std::find(remainingAgents.begin(),
                              remainingAgents.end(),
                              agent);
            ...
            LOG_DEBUG_S << indent
                        << " current coalition structure: " << currentStructure;
            LOG_DEBUG_S << indent << " partial coalition structure: "
                        << coalitionStructure << " , remaining agents "
                        << remainingAgents;

            // Check if we reached the end, i.e. when we have a complete
            // coalition structure
            if(k == partition.size() - 1)
            {
                double currentStructureValue =
                    mCoalitionStructureValueFunction(coalitionStructure);
                LOG_DEBUG_S << indent << " reached the end at k: " << k
                            << " -- bestValue: " << currentBestSolution()
                            << " vs. " << currentStructureValue;
                if(currentStructureValue > currentBestSolutionValue())
                {
                    // update the currently best coalition structure
                    LOG_DEBUG_S
                        << indent
                        << " update global best: " << currentBestSolution()
                        << ", value " << currentBestSolutionValue();
                    improvedResult = updateCurrentBestCoalitionStructure(
                                         coalitionStructure,
            ...
            } else
            {
                ...
                {
                    double valueOfCoalition =
                        mCoalitionValueFunction(coalitionStructure[i]);
                    subspacePotentialValue =
                        std::min(valueOfCoalition, subspacePotentialValue);
                    LOG_DEBUG_S << indent
                                << " coalition: " << coalitionStructure[i]
                                << std::endl
                                << indent << "     value: " << valueOfCoalition
                                << std::endl
                                << indent << "     current subspace potential: "
                                << subspacePotentialValue << std::endl;
                }

                // Estimate value for the rest of the partition based on the
                // bounds computed on coalition sizes, i.e. looking at the
                // integer partition's yet uninvestigated range
                for(size_t i = coalitionStructure.size(); i < partition.size();
                    ++i)
                {
                    double max_s = mCoalitionBoundMap[partition[i]].maximum;
                    LOG_DEBUG_S << indent << " potential for coalition size: "
                                << partition[i] << ": " << max_s;
                    subspacePotentialValue =
                        std::min(max_s, subspacePotentialValue);
                }
                LOG_DEBUG_S
                    << indent
                    << " subspace potential: " << subspacePotentialValue;

                // Check if it is worth to enter this subspace
                if(currentBestSolutionValue() < subspacePotentialValue)
                {
                    LOG_DEBUG_S << indent << " continue search: best: "
                                << currentBestSolutionValue()
                                << " vs. subspace potential: "
                                << subspacePotentialValue;
                    if(searchSubspace(partition,
                                      k + 1,
                                      m_k[0],
                                      remainingAgents,
                                      coalitionStructure,
                ...
                } else
                {
                    LOG_DEBUG_S
                        << indent
                        << " pruning: insufficient subspace potential: best: "
                        << currentBestSolutionValue() << " vs. "
                        << subspacePotentialValue;
                }
            }

            // Stop if the required solution has been found or if the current
            // best is equal to the upper bound of this sub-space
            if(currentBestSolutionQuality() <= betaStar)
            {

                LOG_DEBUG_S << indent
                            << " prematurely returning best structure: "
                            << currentBestSolution();
                LOG_DEBUG_S
                    << "Quality sufficient: " << currentBestSolutionQuality()
                    << "<=" << betaStar;
            ...
            if(currentBestSolutionValue() == upperBoundOfSubspace)
            {
                ...
                LOG_DEBUG_S
                    << indent
                    << " bestStructureValue == upperBoundOfSubspace, both are "
                    << currentBestSolutionValue();
                return improvedResult;
            }
        } else
        {
            ...
            LOG_DEBUG_S << indent
                        << " skipping: condition does not hold: alpha < "
                           "m_k[0]+1 && m_k[0]+1 <= upperBoundM_k"
                        << ", alpha= " << alpha << ", m_k[0]=" << m_k[0]
                        << ", upperBoundM_k=" << upperBoundM_k;
        }
    } while(combinations.next());
...
bool CoalitionStructureGeneration::updateCurrentBestCoalitionStructure(
...
CoalitionStructureGeneration::Statistics
CoalitionStructureGeneration::getStatistics() const
{
    boost::unique_lock<boost::mutex> lock(mStatisticsMutex);
    return mStatistics;
}
