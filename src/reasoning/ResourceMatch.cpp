...
std::string ResourceMatch::Solution::toString(uint32_t indent) const
{
    ...
    std::map<ModelBound, ModelBound::List>::const_iterator cit =
        mAssignments.begin();
...
ModelBound::List
ResourceMatch::Solution::getAssignments(const owlapi::model::IRI& model) const
{
    std::map<ModelBound, ModelBound::List>::const_iterator cit =
        mAssignments.begin();
    for(; cit != mAssignments.end(); ++cit)
    ...
    throw std::invalid_argument("moreorg::reasoning::Resource::Solution::"
                                "getAssignments: no assignments for model '" +
                                model.toString() + "' in solution");
}

ModelBound::List ResourceMatch::Solution::substractMinFrom(
    const ModelBound::List& availableResources) const
{
    ...
    for(; cit != mAssignments.end(); ++cit)
    {
        ...
        for(; ait != assignedModels.end(); ++ait)
        {
            ...
            ModelBound::List::iterator it = std::find_if(
            ...
            if(it == remainingResources.end())
            {
                throw std::invalid_argument(
                    "moreorg::reasoning::ResourceMatch::Solution::"
                    "substractMinFrom: could not find model '" +
                    assignedModelBound.model.toString() + "'");
            }
...
ResourceMatch::ResourceMatch(const ModelBound::List& required,
    ...
    , mModelAssignment(*this,
                       /*width --> col*/ available.size() *
                           /*height --> row*/ required.size(),
                       0,
                                ...
                                ModelBound::getMaxResourceCount(available)))
{
    ...
    Gecode::Matrix<Gecode::IntVarArray> modelAssignment(
        mModelAssignment,
        /*width --> col*/ available.size(),
        /* height --> row*/ required.size());
    ...
    for(size_t ri = 0; ri < mRequiredModelBound.size(); ++ri)
    {
        ...
        for(size_t ai = 0; ai < mAvailableModelBound.size(); ++ai)
        {
            ...
            const owlapi::model::IRI& availableModel =
                availableModelBound.model;

            if(requiredModel == availableModel ||
               ask.isSubClassOf(availableModel, requiredModel))
            {
                LOG_DEBUG_S << "Available model to fulfill '" << requiredModel
                            << std::endl
                            << "    " << availableModel << std::endl
                            << "    maxRequired:" << requiredModelBound.max
                            << std::endl
                            << "    maxAvailable:" << availableModelBound.max
                            << std::endl;
        ...
        LOG_DEBUG_S << "Required instances of model: " << requiredModel
                    << ", minimum: " << requiredModelBound.min;
        // Row requires a minimum of resources to fulfill this particular
        // requirement sum of available (and valid supporting ones) should be at
        // a minimum of the required
    ...
    // branch(*this, mModelAssignment, Gecode::INT_VAR_SIZE_MAX(),
...
Gecode::Space* ResourceMatch::copy() { return new ResourceMatch(*this); }

ResourceMatch::Solution ResourceMatch::solve(
                                             ...
                                             const OWLOntology::Ptr& ontology)
{
    ...
    if(_available.empty())
    {
        throw std::runtime_error(
            "moreorg::reasoning::ResourceMatch::solve: no available models");
    }
...
{
    return solve(modelRequirements,
                 providerResources,
                 ask.getOrganizationModel()->ontology());
}
...
ResourceMatch::Solution ResourceMatch::getSolution() const
{
    Gecode::Matrix<Gecode::IntVarArray> modelAssignment(
        mModelAssignment,
        mAvailableModelBound.size(),
        mRequiredModelBound.size());
    ...
    for(size_t mi = 0; mi < mAvailableModelBound.size(); ++mi)
    {
        ...
        for(size_t i = 0; i < mRequiredModelBound.size(); ++i)
        {
            ...
            {
                throw std::runtime_error(
                    "moreorg::reasoning::ResourceMatch::getSolution: value has "
                    "not been assigned");
            }

            Gecode::IntVarValues v(var);

            ModelBound modelBound(mAvailableModelBound[mi].model,
                                  v.val(),
                                  v.val());
            if(modelBound.min != 0)
            {
...
ResourceMatch* ResourceMatch::solve()
{
    ...
    {
        throw std::runtime_error(
            "moreorg::reasoning::ResourceMatch: no solution found");
    }
...
ModelBound::List ResourceMatch::toModelBoundList(
    const std::vector<owlapi::model::OWLCardinalityRestriction::Ptr>&
        restrictions)
{
    using namespace owlapi::model;
    std::map<owlapi::model::IRI, OWLCardinalityRestriction::MinMax> bounds =
        OWLObjectCardinalityRestriction::getBounds(restrictions);

    ModelBound::List modelBounds;
    std::map<owlapi::model::IRI,
             OWLCardinalityRestriction::MinMax>::const_iterator cit =
        bounds.begin();
    for(; cit != bounds.end(); ++cit)
    {
        modelBounds.push_back(
            ModelBound(cit->first, cit->second.first, cit->second.second));
    }
                                 ...
                                 const owlapi::model::IRI& objectProperty)
{
    ...
    std::vector<OWLCardinalityRestriction::Ptr> providerRestrictions =
        ask.getCardinalityRestrictions(providerModel, objectProperty);
    std::vector<OWLCardinalityRestriction::Ptr> serviceRestrictions =
        ask.getCardinalityRestrictions(serviceModel, objectProperty);

    return isSupporting(providerRestrictions, serviceRestrictions, ontology);
}

bool ResourceMatch::isSupporting(
    const std::vector<owlapi::model::OWLCardinalityRestriction::Ptr>&
    ...
    owlapi::model::OWLOntology::Ptr ontology)
{
    try
    {
        ResourceMatch::Solution fulfillment =
            ResourceMatch::solve(serviceRestrictions,
                                 providerRestrictions,
                                 ontology);
        return true;
...
owlapi::model::IRIList ResourceMatch::filterSupportedModels(
    const ModelPool& modelPool,
    const owlapi::model::IRIList& resourceModels,
    owlapi::model::OWLOntology::Ptr ontology,
    const owlapi::model::IRI& objectProperty)
{
    return filterSupportedModels(modelPool.toModelCombination(),
                                 resourceModels,
                                 ontology,
                                 objectProperty);
}
...
owlapi::model::IRIList ResourceMatch::filterSupportedModels(
    const owlapi::model::IRIList& combinations,
    const owlapi::model::IRIList& resourceModels,
    owlapi::model::OWLOntology::Ptr ontology,
    const owlapi::model::IRI& objectProperty)
{
    OWLOntologyAsk ask(ontology);
    std::vector<OWLCardinalityRestriction::Ptr> providerRestrictions =
        ask.getCardinalityRestrictions(combinations, objectProperty);
    ...
    for(; it != resourceModels.end(); ++it)
    {
        ...
        std::vector<OWLCardinalityRestriction::Ptr> resourceRestrictions =
            ask.getCardinalityRestrictions(resourceModel, objectProperty);
        if(resourceRestrictions.empty())
        {
            ...
            OWLCardinalityRestriction::Ptr restriction =
                OWLCardinalityRestriction::getInstance(
                    ask.getOWLObjectProperty(objectProperty),
                    1,
                    resourceModelExpression,
                    OWLCardinalityRestriction::MIN);
            resourceRestrictions.push_back(restriction);
        }

        LOG_DEBUG_S << "Checking required models for '"
                    << resourceModel.toString() << "'" << std::endl
                    << "restrictions: "
                    << OWLCardinalityRestriction::toString(resourceRestrictions)
                    << std::endl
                    << " vs provided from '" << combinations << "' : "
                    << "restrictions: "
                    << OWLCardinalityRestriction::toString(
                           providerRestrictions);

        if(ResourceMatch::isSupporting(providerRestrictions,
                                       ...
                                       ontology))
        {
            ...
            LOG_DEBUG_S << "Fulfillment given for: " << std::endl
                        ...
                        << "    combination of provider models: "
