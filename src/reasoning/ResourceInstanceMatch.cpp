...
std::string ResourceInstanceMatch::Solution::toString(uint32_t indent) const
{
    ...
    std::map<ModelBound, ResourceInstance::List>::const_iterator cit =
        mAssignments.begin();
...
ResourceInstance::List ResourceInstanceMatch::Solution::getAssignments(
...
ResourceInstanceMatch::ResourceInstanceMatch(
    ...
    for(size_t ri = 0; ri < mRequiredModelBound.size(); ++ri)
    {
        ...
        for(size_t ai = 0; ai < mAvailableResources.size(); ++ai)
        {
            ...
            const owlapi::model::IRI& availableModel =
                availableResource.getModel();

            resourceAssignments[availableResource] << m;

            if(requiredModel == availableModel ||
               ask.ontology().isSubClassOf(availableModel, requiredModel))
            {
                LOG_DEBUG_S << "Available model to fulfill '" << requiredModel
                            ...
                            << std::endl
                            << "    available:" << availableResource.toString()
                            << std::endl;
...
ResourceInstanceMatch::Solution
ResourceInstanceMatch::solve(const ModelBound::List& required,
                             const ResourceInstance::List& available,
                             const OrganizationModelAsk& ask)
{
    ...
    ResourceInstanceMatch* match =
        new ResourceInstanceMatch(required, available, ask);
        ...
        for(size_t i = 0; i < mRequiredModelBound.size(); ++i)
        {
            ...
            if(!var.assigned())
            {
                throw std::runtime_error(
                    "moreorg::reasoning::ResourceInstanceMatch::getSolution: "
            ...
            if(v.val() != 0)
            {
                solution.addAssignment(mRequiredModelBound[i],
                                       mAvailableResources[mi]);
            }
        }
    }
    return solution;
...
ResourceInstanceMatch* ResourceInstanceMatch::solve()
{
    ...
    {
        throw std::runtime_error(
            "moreorg::reasoning::ResourceInstanceMatch: no solution found");
    }
...
ModelBound::List ResourceInstanceMatch::toModelBoundList(
...
ResourceInstanceMatch::Solution::removeAssignmentsFromList(
    ResourceInstance::List& availableList)
{
    ...
    for(const auto pair : mAssignments)
    {
        ...
        {
            auto listIterator = std::find_if(
                available.begin(),
                available.end(),
                [&ri](const ResourceInstance& availableAgent) {
...
bool ResourceInstanceMatch::isSupporting(
    const Agent& agent,
    const owlapi::model::IRI& serviceModel,
    const OrganizationModelAsk& ask,
    const owlapi::model::IRI& objectProperty)
{
    ResourceInstance::List providerResources =
        ask.getRelated(agent, objectProperty);
    std::vector<OWLCardinalityRestriction::Ptr> serviceRestrictions =
        ask.ontology().getCardinalityRestrictions(serviceModel, objectProperty);

    return isSupporting(providerResources, serviceRestrictions, ask);
}

bool ResourceInstanceMatch::isSupporting(
    const ResourceInstance::List& providerResources,
    const std::vector<owlapi::model::OWLCardinalityRestriction::Ptr>&
        serviceRestrictions,
    const OrganizationModelAsk& ask)
{
    try
    {
        ...
        ResourceInstanceMatch::Solution fulfillment =
            ResourceInstanceMatch::solve(requirements, providerResources, ask);
        return true;
