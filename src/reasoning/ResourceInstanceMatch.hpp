...
/**
 * ResourceInstanceMatch allows to search for a valid solution for checking
 ...
 *
 ...
 make_shared<OrganizationModel>("organization-model.owl"); OrganizationModelAsk
 ask(om);
 ...
 ...
 om.ontology().getCardinalityRestrictions(move_to);


 try {
     // Check whether 'move_to' provides a subset of restriction existing for
 'sherpa' csp::ResourceInstanceMatch::Solution solution =
 cs::ResourceInstanceMatch::solve(r_move_to, r_sherpa, ontology); std::cout <<
...
public:
    ...
    public:
        ...
        getAssignments(const owlapi::model::IRI& model) const;
         ...
         */
        ResourceInstance::List
        removeAssignmentsFromList(ResourceInstance::List& assignmentsToRemove);
     ...
     * min/max bounds
     */
    static ResourceInstanceMatch::Solution
    ...
    static bool isSupporting(
        const ResourceInstance::List& providerRestrictions,
        const std::vector<owlapi::model::OWLCardinalityRestriction::Ptr>&
            serviceRestrictions,
        const OrganizationModelAsk& ask);
};
