...
public:
     ...
     * ModelPool
     */
    static ModelPool::Set applyUpperBound(const ModelPool::Set& modelPools,
     ...
     * ModelPool
     */
    static ModelCombinationSet
    applyUpperBound(const ModelCombinationSet& combinations,
                    const ModelPool& upperBounds);
     ...
     */
    static ModelCombinationSet
    applyLowerBound(const ModelCombinationSet& combinations,
     ...
     * ModelPool
     */
    static ModelPool::Set applyLowerBound(const ModelPool::Set& modelPools,
                                          const ModelPool& lowerBound);
     ...
     */
    static ModelCombinationSet
    expandToLowerBound(const ModelCombinationSet& combinations,
                       const ModelPool& lowerBound);
     ...
     * ModelPool
     */
    static ModelPool::Set expandToLowerBound(const ModelPool::Set& modelPools,
                                             const ModelPool& lowerBound);
     ...
     */
    static std::string toString(const std::set<ModelPool>& modelPoolSet,
                                uint32_t indent = 0);
     ...
     * \return stringified object
     */
    static std::string toString(const std::vector<ModelPool>& modelPoolList,
                                uint32_t indent = 0);

    /**
     * Compute all combinations that can be generated from the given model
     * pool (which defines the available resources)
     * \param maxSize If maxSize > 0 then this will be interpreted as maximum
     * size of the combination in terms of allowed model instances that are part
     * of the ModelPool \see numberOfInstances \return Set of modelpool
     ...
     * not exist
     */
    size_t getValue(const owlapi::model::IRI& resource,
...
public:
     ...
     */
    static std::vector<owlapi::model::IRI>
    getModels(const ModelPool& modelPool);
};
...
namespace std {
template <> struct hash<moreorg::ModelPool>
{
