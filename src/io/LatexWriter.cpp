...
std::string LatexWriter::toString(const facades::Robot& robot)
{
    ...
    if(robot.getModelPool().empty())
    {
        ...
        ss << "\\toprule" << std::endl;
        ss << "\\textbf{Property} & \\textbf{Value} \\\\ \\midrule" << std::endl;
        std::string linesep = "\\\\";
...
std::string LatexWriter::toString(const std::vector<facades::Robot>& facades)
{
    ...
    for(const facades::Robot& robot : facades)
    {
        ...
        std::map<std::string, DoublePropertyFunc> dFunctions =
        {
            ...
            {"supply voltage",           std::bind(&facades::Robot::getSupplyVoltage, robot) },
            {"energy capacity",          std::bind(&facades::Robot::getEnergyCapacity, robot)},
            {"nominal power consumption", std::bind(&facades::Robot::getNominalPowerConsumption, robot) },
            {"min accelleration", std::bind(&facades::Robot::getMinAcceleration, robot) },
            {"max accelleration", std::bind(&facades::Robot::getMaxAcceleration, robot) },
            {"nominal accelleration", std::bind(&facades::Robot::getNominalAcceleration, robot) },
            {"min velocity", std::bind(&facades::Robot::getMinVelocity, robot) },
            {"max velocity", std::bind(&facades::Robot::getMaxVelocity, robot) },
            {"nominal velocity", std::bind(&facades::Robot::getNominalVelocity, robot) },
        };
        ...
        if(robot.getModelPool().empty())
        {
            ...
            {
                std::stringstream value;
                value << std::fixed << std::setprecision(2) << robot.getTransportDemand();
                rows["transport consumption"] += " & " + value.str();
            }
            {
                std::stringstream value;
                value << std::fixed << std::setprecision(2) << robot.getTransportCapacity();
