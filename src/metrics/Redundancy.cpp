...
namespace moreorg
{
    namespace metrics
    {
        ...
        double Redundancy::computeSequential(
            ...
            const ModelPool &modelPool, bool sharedUse) const
        {
            std::vector<double> metrics;
        ...
        double Redundancy::computeMetric(
            ...
            const std::vector<OWLCardinalityRestriction::Ptr> &available, double t0,
            double t1) const
        {
            if (required.empty())
            {
            ...
            ModelBound::List modelBoundRemaining =
                ResourceMatch::toModelBoundList(available);
            ModelBound::List modelBoundRequired =
                ResourceMatch::toModelBoundList(required);
            ...
            for (const OWLCardinalityRestriction::Ptr &cRestriction : required)
            {
                ...
                ProbabilityOfFailure survivability(restriction, probabilityDensityFunction,
                                                   fullModelRedundancy);
