...
BOOST_FIXTURE_TEST_CASE(redundancy, RedundancyFixture)
{
    ...
    BOOST_REQUIRE_MESSAGE(redundancyVal == expected,
                          "Redundancy expected: " << expected << " but got "
                                                  << redundancyVal);
}
...
BOOST_FIXTURE_TEST_CASE(redundancy_same_parallel_in_series, RedundancyFixture)
{
    ...
    // Resources
    {
        ...
        ResourceInstance::List available = ask.getRelated(agent);
        // Redundancy redundancy(ask, make_shared<pdfs::ConstantPDF>(0.5), has);
        //// when the structure be considered then: (1 -pow(
        ///1-pow(0.5,2),2))*pow(0.5,2);
        // double expected = 0.25;
        // double redundancyVal = redundancy.computeMetric(query, available);
        // BOOST_REQUIRE_MESSAGE(redundancyVal == expected, "Redundancy
        // expected: " << expected << " but got " << redundancyVal);
    }
}
...
BOOST_AUTO_TEST_CASE(redundancy_computation)
{
    ...
    OrganizationModelAsk omAsk(
        OrganizationModel::Ptr(new OrganizationModel(om)));
    ...
    //    redundancy.computeSequential(functionalities, availableModelPool));
    //}

    IRI stereoCameraProvider =
        moreorg::vocabulary::OM::resolve("StereoImageProvider");
    {
        ...
        BOOST_TEST_MESSAGE(
            "TEST: " << redundancy.computeSequential(functionalities,
                                                     available));
    }
    ...
    //    BOOST_REQUIRE_THROW( redundancy.computeExclusiveUse(requiredModelPool,
    //    availableModelPool), std::runtime_error );

    //    availableModelPool[sherpa] = 2;
    //    double r =  redundancy.computeExclusiveUse(requiredModelPool,
    //    availableModelPool);

    //    BOOST_TEST_MESSAGE("Test show required: " <<
    //    OWLCardinalityRestriction::toString(
    //    omAsk.getCardinalityRestrictions(requiredModelPool,
    //    moreorg::vocabulary::OM::has(), OWLCardinalityRestriction::SUM_OP) )
    //    ); BOOST_TEST_MESSAGE("Test show available: " <<
    //    OWLCardinalityRestriction::toString(
    //    omAsk.getCardinalityRestrictions(availableModelPool,
    //    moreorg::vocabulary::OM::has(), OWLCardinalityRestriction::SUM_OP) )
    ...
    //    // todo: redundancy when nothing is required
    //    double r = redundancy.computeSharedUse(requiredModelPool,
    //    availableModelPool); BOOST_REQUIRE_MESSAGE(true, "Redundancy shared
    //    use is:" << r);
    //}
}
...
BOOST_AUTO_TEST_CASE(function_redundancy)
{
...

    {
        ...
        BOOST_REQUIRE_MESSAGE(value != 0,
                              "TransportProvider has redundancy: " << value);
    }
}
...
BOOST_AUTO_TEST_CASE(probability_density_function)
{
    ...
    ProbabilityDensityFunction::Ptr pdf_ptr =
        ProbabilityDensityFunction::getInstance(
            ask,
            IRI("http://www.rock-robotics.org/2015/12/robots/"
                "Sherpa#Sherpa_Camera_Front"));
    BOOST_REQUIRE_MESSAGE(pdf_ptr,
                          "Probability Density Function has been created");

    BOOST_TEST_MESSAGE(
        "Probability Density Value at t = 0: " << pdf_ptr->getValue());

    ProbabilityDensityFunction::Ptr weibull_pdf_ptr =
        make_shared<pdfs::WeibullPDF>(36000., 1.); // 36000s = 10h
    BOOST_TEST_MESSAGE("Weibull Probability Density Value at t = 0: "
                       << weibull_pdf_ptr->getValue());
    BOOST_TEST_MESSAGE(
        "Weibull Conditional Probability Value at t0 = 0, t1 = 0: "
        << weibull_pdf_ptr->getConditional());
}
...
BOOST_FIXTURE_TEST_CASE(probability_of_failure, RedundancyFixture)
{
    ...
    reasoning::ModelBound requirement(
        moreorg::vocabulary::OM::resolve("Sherpa"),
        1,
        1);
    ...
    ProbabilityDensityFunction::Ptr pdf_ptr =
        make_shared<pdfs::WeibullPDF>(36000., 1.);

    ProbabilityOfFailure PoF(requirement, available, pdf_ptr);
    BOOST_TEST_MESSAGE(
        "Probability of Failure Conditional Value at t0 = 0, t1 = 0: "
        << PoF.getProbabilityOfFailureConditionalWithRedundancy(0., 0.));
}
...
BOOST_AUTO_TEST_CASE(metric_map_computation)
{
    ...
    //    {
    //        IRI
    //        locationImageProvider("http://www.rock-robotics.org/2014/01/om-schema#ImageProvider");
    //        IRI
    //        sherpa("http://www.rock-robotics.org/2014/01/om-schema#Sherpa");
    //
    //        ModelPool modelPool;
    //        for(int i = 1; i <= 3; ++i)
    //        {
    //            modelPool[sherpa] = i;
    //            double pSurvivability =
    //            redundancy.compute(locationImageProvider, modelPool);
    ...
    //    {
    //        IRI
    //        locationImageProvider("http://www.rock-robotics.org/2014/01/om-schema#LocationImageProvider");
    ...
    //            //            ModelBound::List
    //            //            ModelBound:
    //            'http://www.rock-robotics.org/2014/01/om-schema#Camera' (1,
    //            1000000)
    //            //            ModelBound:
    ...
    //            (1, 1000000)
    //            //            ModelBound:
    //            'http://www.rock-robotics.org/2014/01/om-schema#Mapping' (1,
    //            1000000)
    //            //            ModelBound:
    ...
    //            //            ModelBound::List
    //            //            ModelBound:
    //            'http://www.rock-robotics.org/2014/01/om-schema#Camera' (0, 3)
    //            //            ModelBound:
    ...
    //            (0, 3)
    //            //            ModelBound:
    //            'http://www.rock-robotics.org/2014/01/om-schema#Mapping' (0,
    //            3)
    //            //            ModelBound:
    //            'http://www.rock-robotics.org/2014/01/om-schema#PowerSource'
    //            (0, 3)
    ...
    //            double pSurvivability =
    //            redundancy.compute(locationImageProvider, modelPool);
    //            BOOST_REQUIRE_MESSAGE(pSurvivability == expected,
    //            "Survivability for #" << i << " " << crex << ": \n" <<
    ...
    //        locationImageProvider("http://www.rock-robotics.org/2014/01/om-schema#LocationImageProvider");
    //        IRI
    //        stereoCamera("http://www.rock-robotics.org/2014/01/om-schema#StereoImageProvider");
    //
    //        IRISet services;
    ...
    //
    //        IRI
    //        robot("http://www.rock-robotics.org/2014/01/om-schema#Sherpa");
    //        ModelPool modelPool;
    //        for(int i = 1; i <= 3; ++i)
    //        {
    //            modelPool[robot] = i;
    ...
    //            double expected = pow(parallel,5);
    //
    //            double pSurvivability = redundancy.computeSharedUse(services,
    //            modelPool); BOOST_TEST_MESSAGE("Survivability for #" << i << "
    //            " << robot << ": \n" << pSurvivability);
    //            //BOOST_REQUIRE_MESSAGE(pSurvivability == expected,
    //            "Survivability for #" << i << " " << crex << ": \n" <<
    //            pSurvivability << " (expected: " << expected << ")");
