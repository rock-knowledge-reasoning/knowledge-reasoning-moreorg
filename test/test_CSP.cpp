...
//    OWLOntology::Ptr ontology = io::OWLOntologyIO::fromFile( getRootDir() +
//    "/test/data/om-schema-v0.6.owl"); ontology->refresh();
//
...
//    tell.subclassOf(b,a);
//    ontology->refresh();
//    BOOST_REQUIRE_MESSAGE(ask.isSubClassOf(c->getIRI(), b->getIRI()), "C
//    should be subclass of b");
//
...
//
//    ResourceMatch* match = ResourceMatch::solve(query, resourcePool,
//    ontology);
//
//    BOOST_TEST_MESSAGE("Assignment: " << match->toString());
//
//    IRI assignment = match->getAssignment(a->getIRI());
//    BOOST_REQUIRE_MESSAGE( assignment == b->getIRI(), "Expected base to be
//    matched by base-derived");
//}
...
BOOST_AUTO_TEST_CASE(match_resource_via_restrictions)
{
    ...
    OWLObjectProperty::Ptr property =
        tell.objectProperty("http://property/has");

    tell.subClassOf(c, b);
    tell.subClassOf(b, a);
    ontology->refresh();
    BOOST_REQUIRE_MESSAGE(ask.isSubClassOf(c->getIRI(), b->getIRI()),
                          "C should be subclass of b");
    ...
    ResourceMatch::Solution solution =
        ResourceMatch::solve(query, resourcePool, ontology);
...
BOOST_AUTO_TEST_CASE(provider_via_restrictions)
{
    ...
    IRI stereo_image_provider =
        moreorg::vocabulary::OM::resolve("StereoImageProvider");
    IRI location_image_provider =
        moreorg::vocabulary::OM::resolve("LocationImageProvider");

    std::vector<OWLCardinalityRestriction::Ptr> r_sherpa =
        ask.getCardinalityRestrictions(sherpa, moreorg::vocabulary::OM::has());
    std::vector<OWLCardinalityRestriction::Ptr> r_move_to =
        ask.getCardinalityRestrictions(move_to, moreorg::vocabulary::OM::has());
    std::vector<OWLCardinalityRestriction::Ptr> r_image_provider =
        ask.getCardinalityRestrictions(image_provider,
                                       moreorg::vocabulary::OM::has());
    std::vector<OWLCardinalityRestriction::Ptr> r_stereo_image_provider =
        ask.getCardinalityRestrictions(stereo_image_provider,
                                       moreorg::vocabulary::OM::has());
    std::vector<OWLCardinalityRestriction::Ptr> r_location_image_provider =
        ask.getCardinalityRestrictions(location_image_provider,
                                       moreorg::vocabulary::OM::has());

    using namespace moreorg::reasoning;
    ResourceMatch::Solution fulfillment =
        ResourceMatch::solve(r_move_to, r_sherpa, ontology);
    BOOST_TEST_MESSAGE(
        "Sherpa provides MoveTo\nAssignment: " << fulfillment.toString());
    BOOST_TEST_MESSAGE("MoveTo requirements: "
                       << OWLCardinalityRestriction::toString(r_move_to));
    BOOST_TEST_MESSAGE(
        "Sherpa provides: " << OWLCardinalityRestriction::toString(r_sherpa));
    {
        std::string capabilityName = "SelfLocalization";
        ModelBound::List modelBounds = fulfillment.getAssignments(
            moreorg::vocabulary::OM::resolve(capabilityName));
        BOOST_REQUIRE_MESSAGE(
            modelBounds.size() == 1,
            "Matching of 1 provided resource expected, but got '"
                << modelBounds.size());

        ModelBound modelBound = modelBounds.front();
        BOOST_REQUIRE_MESSAGE(modelBound.min == 1 && modelBound.max == 1,
                              "Model bound for '"
                                  << modelBound.model
                                  << "', expected min 1, max 1, but got: "
                                  << modelBound.toString());
    }
    {
    ...
    {
        OWLObjectProperty::Ptr hasProperty =
            ask.getOWLObjectProperty(moreorg::vocabulary::OM::resolve("has"));
        OWLClass::Ptr c_image_provider = ask.getOWLClass(image_provider);
        OWLCardinalityRestriction::Ptr restriction =
            make_shared<OWLObjectMinCardinality>(hasProperty,
                                                 1,
                                                 c_image_provider);
    ...
    fulfillment =
        ResourceMatch::solve(r_stereo_image_provider, r_sherpa, ontology);
    ...
    //  getCardinalityRestrictions(... , filter on qualification, e.g.
    //  'has','provide')
    //
    //  restriction tree
    ...
    //    --> 1 - move_to
    //    --> 1 - image_provider
    std::vector<OWLCardinalityRestriction::Ptr> r_sherpa_with_service =
        ask.getCardinalityRestrictions(sherpa, moreorg::vocabulary::OM::has());
    try
    {
        fulfillment = ResourceMatch::solve(r_location_image_provider,
                                           r_sherpa_with_service,
                                           ontology);
        BOOST_REQUIRE_MESSAGE(
            true,
            "Sherpa provides LocationImageProvider\nAssignment: "
                << fulfillment.toString());
    } catch(...)
    {
        BOOST_TEST_MESSAGE("Sherpa does not provide "
    ...
    IRI emi_power_provider =
        moreorg::vocabulary::OM::resolve("EmiPowerProvider");
    serviceModels.push_back(emi_power_provider);

    {
        owlapi::model::IRIList supportedModels =
            ResourceMatch::filterSupportedModels(combinedSystem,
                                                 serviceModels,
                                                 ontology);
        BOOST_REQUIRE_MESSAGE(supportedModels.size() == 4,
                              "Services supported by sherpa");
    }

    {
        ...
        BOOST_REQUIRE_MESSAGE(supportedModels.size() == 4,
                              "Services supported by sherpa: computing time: "
...
BOOST_AUTO_TEST_CASE(resource_instance_matching)
{
    ...
    ModelBound::List requiredModelBound =
    ...
    } catch(const std::exception& e)
    {
        BOOST_REQUIRE_MESSAGE(false,
                              "ResourceInstance matching failed " << e.what());
    }
}

BOOST_AUTO_TEST_CASE(performance_three_sherpa)
{
    OWLOntology::Ptr ontology = io::OWLOntologyIO::fromFile(
        getRootDir() + "/test/data/om-schema-v0.6.owl");
    ontology->refresh();
...

    {
        ...
        owlapi::model::IRIList supportedModels =
            ResourceMatch::filterSupportedModels(combinedSystem,
                                                 ...
                                                 ontology);
        base::Time stopTime = base::Time::now();
        BOOST_REQUIRE_MESSAGE(supportedModels.size() == serviceModels.size(),
                              "Services supported by sherpa: computing time: "
                                  << (stopTime - startTime).toSeconds());
    }
}
...

    {
        ...
        owlapi::model::IRIList supportedModels =
            ResourceMatch::filterSupportedModels(combinedSystem,
                                                 ...
                                                 ontology);
        base::Time stopTime = base::Time::now();
        BOOST_REQUIRE_MESSAGE(supportedModels.size() == 1,
                              "Services supported by sherpa: computing time: "
                                  << (stopTime - startTime).toSeconds());
    }
}
...
{
    {
        ...
        ModelBound::List::iterator it =
            std::find_if(list.begin(), list.end(), [](const ModelBound& b) {
                return b.model == "m0";
            });
        BOOST_REQUIRE_MESSAGE(it != list.end(), "ModelBound m0 exists in list");

        ModelBound mb = *it;
        BOOST_REQUIRE_MESSAGE(mb.min == 0 && mb.max == 0,
                              "Substract to null, but got: " << mb.toString());
    }
    {
        ModelBound::List a;
        ...
        ModelBound mb = *it;
        BOOST_REQUIRE_MESSAGE(mb.min == 2 && mb.max == 7,
                              "Substract to null, but got: " << mb.toString());
    }
    {
        ModelBound::List a;
        ...
        BOOST_REQUIRE_MESSAGE(
            mb.min == 2 && mb.max == 7,
            "SubstractMin expected (2,7), but got: " << mb.toString());
    ...
    }
    {
        ...
        ModelBound::List list;
        BOOST_REQUIRE_THROW(list = ModelBound::substractMin(b, a),
    ...
    }
    {
        ...
        ModelBound::List list;
        BOOST_REQUIRE_NO_THROW(list = ModelBound::substractMin(a, b));
        BOOST_REQUIRE_MESSAGE(list.front().min == 0 && list.front().max == 0,
                              "SubstractMin expected (0,0), but got "
                                  << list.front().toString());
    }
    {
        ModelBound::List a;
        ...
        ModelBound::List list;
        BOOST_REQUIRE_THROW(list = ModelBound::substractMin(a, b),
                            std::invalid_argument);
    }
}
...
BOOST_AUTO_TEST_CASE(function_mapping)
{
    ...
    ResourceMatch::Solution solution =
        ResourceMatch::solve(required, available, ask);
    BOOST_TEST_MESSAGE("Solution found: " << solution.toString());
}
